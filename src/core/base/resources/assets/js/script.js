...
class Haitech {
    constructor() {
        ...
        Haitech.handleCounterUp();
        Haitech.initMediaIntegrate();
        Haitech.initSideBarScroll();
        if (HaitechVariables && HaitechVariables.authorized === '0') {
    ...
    static showNotice(messageType, message, messageHeader = '') {
        ...
        if (!messageHeader) {
            switch (messageType) {
                case 'error':
                    messageHeader = HaitechVariables.languages.notices_msg.error;
                    break;
                case 'success':
                    messageHeader = HaitechVariables.languages.notices_msg.success;
                    break;
            }
        }
    ...
    static handleError(data) {
        if (typeof (data.errors) !== 'undefined' && !_.isArray(data.errors)) {
            Haitech.handleValidationError(data.errors);
        } else {
            if (typeof (data.responseJSON) !== 'undefined') {
                if (typeof (data.responseJSON.errors) !== 'undefined') {
                    if (data.status === 422) {
                        Haitech.handleValidationError(data.responseJSON.errors);
                    }
                } else if (typeof (data.responseJSON.message) !== 'undefined') {
                    Haitech.showError(data.responseJSON.message);
                } else {
                    $.each(data.responseJSON, (index, el) => {
                        $.each(el, (key, item) => {
                            Haitech.showError(item);
                        });
                    });
                }
            } else {
                Haitech.showError(data.statusText);
            }
        }
    }
    ...
    countCharacter() {
        $.fn.charCounter = function (max, settings) {
            ...
            settings = $.extend({
                ...
                format: '(%1 ' + HaitechVariables.languages.system.character_remain + ')',
                pulse: true,
                delay: 0
            }, settings);
            let p, timeout;
    ...
    static initDatePicker(element) {
        if (typeof SimplePicker !== 'undefined') {
            ...
            Haitech.initSimplePicker(element);
    ...
    static initResources() {
        ...
        Haitech.initDatePicker('.datepicker');
        if (jQuery().mCustomScrollbar) {
            Haitech.callScroll($('.list-item-checkbox'));
        }
    ...
    handlePortletTools() {
        ...
        $('body').on('click', '.portlet > .portlet-title .fullscreen', event => {
            ...
            if (portlet.hasClass('portlet-fullscreen')) {
                ...
                let height = Haitech.getViewPort().height -
    ...
    static initCodeEditor(id, type = 'css') {
        ...
        $('.handle-tool-drag').mousedown(event => {
            ...
            $('body').attr('data-dragtool', _self.attr('id')).on('mousemove', Haitech.onDragTool);
            $(window).on('mouseup', Haitech.onReleaseTool);
        });
    }
    ...
    static onReleaseTool() {
        $('body').off('mousemove', Haitech.onDragTool);
        $(window).off('mouseup', Haitech.onReleaseTool);
    }
